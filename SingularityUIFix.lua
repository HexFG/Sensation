--[[
	...
	by Singularity
        ...
        Singularity#5490
  edited by Hex
      added inputs and fixed some bugs.
--]]
...
do -- Load items
    ...
    local inputTextUICorner = Instance.new("UICorner")
    local Shadow_4 = Instance.new("ImageLabel")
    local inputTextPadding = Instance.new("UIPadding")
    ...
    inputText.Parent = Presets
    inputText.BackgroundColor3 = Color3.fromRGB(41, 74, 122)
    inputText.ClipsDescendants = false
    inputText.Size = UDim2.new(1, 0, 0, 20)
    inputText.Font = Enum.Font.Code
    inputText.PlaceholderColor3 = Color3.fromRGB(184, 184, 184)
    inputText.PlaceholderText = ""
    inputText.Text = ""
    inputText.TextColor3 = Color3.fromRGB(255, 255, 255)
    inputText.TextSize = 12.000
    inputText.TextXAlignment = Enum.TextXAlignment.Left

    inputTextUICorner.Parent = inputText
    inputTextUICorner.CornerRadius = UDim.new(0, 4)

    inputTextPadding.Parent = inputText
    inputTextPadding.PaddingLeft = UDim.new(0, 5)

    Shadow_4.Name = "Shadow"
    Shadow_4.Parent = inputText
    Shadow_4.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    Shadow_4.BackgroundTransparency = 1.000
    Shadow_4.Position = UDim2.new(0, 2, 0, 2)
    Shadow_4.Size = UDim2.new(1, 1, 1, 1)
    Shadow_4.ZIndex = 0
    Shadow_4.Image = "rbxassetid://3570695787"
    ...
    new = function(options)
        ...
        function self.new(tabOptions)
            ...
            local types = { } do
                ...
                function types.input(textInputOptions)
                    local self = { }
                    self.eventBlock = false

                    textInputOptions = settings.new({
                        text = "",
                        placeholder = ""
                    }).handle(textInputOptions)

                    local textInput = new("textInput")
                    textInput.Parent = items
                    textInput.Text = textInputOptions.text
                    textInput.PlaceholderText = textInputOptions.placeholder

                    textInput:GetPropertyChangedSignal("Text"):Connect(function()
                        if not self.eventBlock then
                            self.event:Fire(textInput.Text)
                        end
                    end)
